...
const char* Player::getStateName(PlayerState state) {
    switch (state) {
        case PlayerState::UNLOADED: return "UNLOADED";
        case PlayerState::READY:    return "READY";
        case PlayerState::PLAYING:  return "PLAYING";
        case PlayerState::PAUSE:   return "PAUSED";
        case PlayerState::TEST:     return "TEST";
        default:                    return "UNKNOWN";
    }
}
    ...
    switch (m_state) {
        case PlayerState::UNLOADED:
            if (e.type == EVENT_LOAD) {
                if (acquireResources() == ESP_OK) switchState(PlayerState::READY);
                else ESP_LOGE(TAG, "resource acquire failed");
            }
            break;

        case PlayerState::READY:
            if (e.type == EVENT_PLAY) switchState(PlayerState::PLAYING);
            ...
            else ESP_LOGW(TAG, "ReadyState: ignoring event %s", getEventName(e.type));
            break;
        ...
        case PlayerState::PAUSE:
            ...
            else ESP_LOGW(TAG, "PauseState: ignoring event %s", getEventName(e.type));
            break;
